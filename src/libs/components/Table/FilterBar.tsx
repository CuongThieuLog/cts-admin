...
import { ButtonCreate, ButtonExportCsv, ButtonSearch } from './styled'
...
export type FilterColumn<F extends FieldValues> = FilterColumnBase<F> &
  (InputFilterColumn | SelectFilterColumn<F> | CustomFilterColumn<F>)
...
export function FilterBar<FilterInput extends FieldValues, TData>({
  ...
  const hasExportCsvFn = exportCsv && typeof exportCsv.onExportCsv === 'function'
  const hasOnBack = typeof onBack === 'function'
  const hasCustomButton = !!customButton
  ...
  return (
    ...
    >
      ...
      <Stack spacing={2} direction="row">
        ...
        {hasOnBack && (
          <ButtonSearch variant="outlined" onClick={onBack}>
            Quay lại
          </ButtonSearch>
        )}
        ...
        {hasExportCsvFn && (
          <ButtonExportCsv
            variant="outlined"
            startIcon={<Image src="/assets/imgs/csv.png" width={20} height={20} alt="csv" />}
            onClick={!exportCsv.isLoading ? exportCsv.onExportCsv : undefined}
          >
            {exportCsv.isLoading ? 'Đang xuất file...' : 'Xuất file excel'}
          </ButtonExportCsv>
        )}
